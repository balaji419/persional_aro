terraform {
  ...
  backend "azurerm" {
    storage_account_name = "azurearodev"
...
resource "azurerm_role_assignment" "vnet_assignment" {
  ...
  scope			= var.aro_vnet_name
  role_definition_name	= var.roles[count.index].role
  principal_id		= module.app_registration.client_id
}
...
resource "azurerm_role_assignment" "rp_assignment" {
  ...
  scope			= var.aro_vnet_name
  role_definition_name	= var.roles[count.index].role
  principal_id		= module.app_registration.object_id
}
...
resource "azurerm_template_deployment" "azure-arocluster" {
  ...
  parameters 		= {
    ...
    aadClientId			= module.app_registration.client_id
    aadClientSecret		= module.app_registration.client_secret

    clusterVnetId		= var.aro_vnet_name
    workerSubnetId		= var.aro_worker_subnet_name
    masterSubnetId		= var.aro_master_subnet_name
...
resource "null_resource" "azuread-connect" {
  provisioner "local-exec" {
    command 		= <<EOC
      ...
      export CLIENT_ID= module.app_registration.client_id
      export CLIENT_SECRET= module.app_registration.client_secret
